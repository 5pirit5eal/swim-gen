...
export function parseContentForDrillLinks(content: string): ContentSegment[] {
    ...
    while ((match = MARKDOWN_LINK_REGEX.exec(content)) !== null) {
        ...
        console.debug('[markdownParser] Parsed: fullMatch=', fullMatch, 'linkText=', linkText, 'url=', url, 'index=', matchIndex)
        ...
        if (matchIndex > lastIndex) {
            ...
            if (textBefore) {
                console.debug('[markdownParser] Adding text segment before match:', JSON.stringify(textBefore))
