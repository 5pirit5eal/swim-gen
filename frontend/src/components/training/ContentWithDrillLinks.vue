...
const segments = computed<ContentSegment[]>(() => {
    console.debug('[ContentWithDrillLinks] Computing segments for content:', JSON.stringify(props.content))
...
<template>
    <span class="content-with-drill-links">
        <template v-for="(segment, index) in segments" :key="index">
            ...
            <DrillLink v-else-if="segment.type === 'drill-link'" :drill-id="segment.drillId" :text="segment.text" />
        </template>
    </span>
</template>
